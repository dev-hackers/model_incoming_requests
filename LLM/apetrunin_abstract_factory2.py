"""
Задача:
1.Разработать абстрактный класс Street в котором будет 
возможность регистрировать произвольное число объектов относящихся к этому классу.

2. Разработать абстрактный класс AbstractStreet в котором будут реализованы методы:
- Добавление экземпляра объекта в список принадлежащих объектов 
для текущего экземпляра Street.
- Удаление экземпляра объекта из списка принадлежащих объектов 
для текущего экземпляра Street.
...
3. Реализовать абстрактный класс AbstractObject 
на основе которого будут создаваться конкретные классы объектов, 
которые будут принадлежать классу Street. 
...
5. Реализовать три конкретных класса House, Shopping, 
Station на основе абстрактного класса AbstractHouse.
"""
...
class Street(AbstractStreet):
    ...
    def del_object(self, description: str):
        ...
        """
        exists = [object for object in self._street_inventory if object.description == description]
        if exists:
            self._street_inventory.remove(object[0])
        else:
